...
function DockItem({
  ...
  // Calculate smooth cosine bell-curve magnification factor
  let scale = 1;
  if (mouseX !== null && itemRef.current) {
    ...
    if (dist < distance) {
      ...
      // Cosine interpolation for organic Apple-like magnification curve
      scale = 1 + (maxScale - 1) * Math.cos(normDist * (Math.PI / 2));
    }
  }

  const currentSize = baseSize * scale;
  const iconSize = Math.max(16, Math.round(18 * (scale * 0.85)));
  ...
  return (
    ...
    >
      ...
      {showTooltips && isHovered && (
        <div
          className="absolute -top-10 left-1/2 -translate-x-1/2 px-2.5 py-1 rounded-xl bg-black/85 dark:bg-[#1A1A1D]/95 text-white font-mono text-[10px] font-semibold tracking-wide whitespace-nowrap shadow-xl border border-white/15 backdrop-blur-md pointer-events-none animate-fadeIn z-30 flex items-center gap-1"
        >
      ...
      >
        ...
        <div
          ...
          style={{
            background: `radial-gradient(circle at center, ${accentColor}30 0%, transparent 70%)`,
          }}
        />

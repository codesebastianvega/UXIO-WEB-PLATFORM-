...
interface BrandDiagnosisCardProps {
...
export default function BrandDiagnosisCard({
  submission,
  score5,
  courseSlug,
  lang,
  onOpenModal,
}: BrandDiagnosisCardProps) {
  const isEs = lang === 'es';
  ...
  return (
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            >
              {isApproved
                ...
                : (isEs ? 'Formulario Interactivo (3 Bloques)' : 'Interactive Worksheet (3 Blocks)')}
            </span>
            <FileText
              size={14}
              className={
          ...
          <h4 className="font-display font-bold text-sm text-[#111111] dark:text-white">
            {isEs ? 'Ficha de Diagnóstico de Marca' : 'Brand Diagnosis Worksheet'}
          </h4>
          <p className="text-xs text-[#666666] dark:text-[#8E8E93] font-sans">
            {isEs
              ? 'Abre el documento interactivo para definir tu oferta, 3 dolores y meta de 5 semanas.'
              : 'Open interactive worksheet to declare your offer, 3 pain points, and 5-week goal.'}
          </p>
        </div>
            ...
            <span>
              {isSubmitted || isApproved
                ? isEs
                  ...
                  : 'View / Edit Sheet'
                : isEs
                ? 'Abrir Ficha de Diagnóstico ➔'
                : 'Open Diagnosis Sheet ➔'}
            </span>
          </button>
        </div>
      </div>
      ...
      {submission?.feedbackText && (
        <div className="p-3.5 rounded-xl bg-black/[0.03] dark:bg-white/[0.03] border border-black/[0.06] dark:border-white/[0.06] text-xs space-y-1">
          <span className="font-mono text-[10px] font-bold text-[#10B981] uppercase block">
            // {isEs ? 'FEEDBACK DEL DOCENTE' : 'INSTRUCTOR FEEDBACK'}
          </span>
          <p className="italic text-[#333333] dark:text-[#E4E4E7]">"{submission.feedbackText}"</p>
        </div>
      )}
    </div>
  );
